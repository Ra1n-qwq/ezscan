        ...
        if result != "" and result != None:
            ...
            def info_pr(mc, text):
                ...
                if len(mc_info) > 0:
                    for i in mc_info:
                        match_info.append(i)
                    # print("[INFO] " + text + " %s " % ", ".join(str(i) for i in mc_info))
                    file.write(text + " %s " % ", ".join(str(i) for i in mc_info) + '\n')
                    print(text + " %s " % ", ".join(str(i) for i in mc_info) + '\n')
            ...
            for i in other_info:
                if len(i) > 40:
                    other_info.pop()
            # print("\[INFO] Other: %s" % ", ".join(str(i) for i in list(set(other_info))))
            file.write("Other: %s" % ", ".join(str(i) for i in list(set(other_info))) + '\n')
            file.write('\n')
            file.close()
            print("Other: %s" % ", ".join(str(i) for i in list(set(other_info))) + '\n')
            print("\n")

    def one_arr_res(self, target_url):
        info = []
        version_info = []
        result = self.analyzer(target_url=target_url)#获取banner信息
        # print(result)
        #sys.exit()
        if result != "" and result != None:#提取获得的信息
            for i in result:
                name = i['name']
                origin = i['origin']
                try:
